...
sys.path.append(join(dirname(dirname(__file__)), "model"))
...
def eval_model(args):
    ...
    caption_data = []
    # caption_file = "MVP/output/coco_pope_popular_caption_llava_{}.jsonl".format(
    #     args.perspective
    # )
    ...
    for i, line in enumerate(tqdm(questions)):
        ...
        with torch.inference_mode():
            ...
            predicted_tokens = (
                tokenizer.decode(top_k_indices.tolist(), skip_special_tokens=True)
                .strip()
                .split()
            )
            ...
            filtered_tokens = [
                token for token in predicted_tokens if token not in ["Yes", "No"]
            ]
            indices = [
                i
            ...
            for token, value in zip(filtered_tokens, filtered_values):
                ...
                input_ids = (
                    tokenizer_image_token(
                        new_prompt, tokenizer, IMAGE_TOKEN_INDEX, return_tensors="pt"
                    )
                    .unsqueeze(0)
                    .cuda()
                )
                ...
                answer_probabilities = torch.softmax(
                    output_scores[answer_index][0], dim=-1
                )
                ...
                if answer_lower in ["yes", "no"]:
                    ...
                    if answer_lower == "yes":
                        ...
                        aggregation_dict["Yes"] += value * diff_prob
                    elif answer_lower == "no":
                        # no_aggretion_logits += value * diff_prob
                        aggregation_dict["No"] += value * diff_prob
                else:
                    print(f"Unexpected answer token: {answer}")
                    # import ipdb; ipdb.set_trace()

            final_answer = (
                "Yes" if aggregation_dict["Yes"] > aggregation_dict["No"] else "No"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--image-folder",
        type=str,
        default="/home/rise/sdc2T/XJH/FEMA-Inspection/data/POPE-demo",
    )
    parser.add_argument(
        "--question-file",
        type=str,
        default="/home/rise/sdc2T/XJH/FEMA-Inspection/data/POPE-demo/coco-pope-demo.jsonl",
    )
    parser.add_argument(
        "--caption-file",
        type=str,
        default="/home/rise/sdc2T/XJH/FEMA-Inspection/data/POPE-demo/caption-bottom-up.jsonl",
    )
    parser.add_argument(
        "--answers-file",
        type=str,
        default="/home/rise/sdc2T/XJH/FEMA-Inspection/data/POPE-demo/mvp-answer-bottom-up.jsonl",
    )
    parser.add_argument(
        "--perspective", type=str, default="bottom-up"
    )  # bottom-up, top-down, regular
